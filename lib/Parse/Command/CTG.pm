...
sub abstract {
	return "parse CTG (geographical call trace) files from Alcatel-Lucent WNMS into csv files (experimental)";
}

sub usage_desc {
	return "%c CTG %o [ filename(s) ]";
}

sub opt_spec {
	return (
	[ "outdir|d=s",	"directory to store parsed csv file(s) in", { default => "../csvload" }],
	[ "table|t=s", "table name", {hidden => 1, default => 'TRACE_CTG'}],
